...
const TYPE_STYLE: Record<WorkoutType, { bg: string; border: string; text: string; dot: string }> = {
  run:      { bg: 'bg-blue-50',   border: 'border-blue-200',   text: 'text-blue-800',   dot: 'bg-blue-400' },
  bike:     { bg: 'bg-orange-50', border: 'border-orange-200', text: 'text-orange-800', dot: 'bg-brand-400' },
  swim:     { bg: 'bg-cyan-50',   border: 'border-cyan-200',   text: 'text-cyan-800',   dot: 'bg-cyan-400' },
  strength: { bg: 'bg-purple-50', border: 'border-purple-200', text: 'text-purple-800', dot: 'bg-purple-400' },
  rest:     { bg: 'bg-zinc-50',   border: 'border-zinc-200',   text: 'text-zinc-400',   dot: 'bg-zinc-300' },
  other:    { bg: 'bg-green-50',  border: 'border-green-200',  text: 'text-green-800',  dot: 'bg-green-400' },
}
...
function WeekCalendar({ week }: WeekCalendarProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Summary / rationale */}
      {week.summary && (
        <div className="prose prose-sm prose-zinc max-w-none px-1 text-zinc-500">
            ...
            <div key={day} className="flex flex-col gap-1">
              ...
              <button
                ...
                className={clsx(
                  ...
                  'min-h-[80px] w-full',
                  workout ? [style.bg, style.border, 'hover:shadow-sm cursor-pointer'] : 'bg-zinc-50 border-zinc-100 cursor-default',
                  isExpanded && 'ring-2 ring-brand-400 ring-offset-1',
                )}
              >
                {workout ? (
                  <>
...
export function PlanView({ plan, selectedWeek, onWeekChange }: PlanViewProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {weeks.length > 0 && (
        <div className="flex items-center gap-1 px-4 py-2 border-b border-zinc-100 bg-white shrink-0 overflow-x-auto scrollbar-thin">
          <button
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 scrollbar-thin">
        <div className="mb-3">
          <h2 className="text-base font-semibold text-zinc-800">{current.title}</h2>
        </div>
        <WeekCalendar week={current} />
      </div>

      {/* Footer */}
      <div className="shrink-0 px-6 py-2 border-t border-zinc-100 bg-zinc-50 text-xs text-zinc-400">
