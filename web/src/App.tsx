...
export default function App() {
  ...
  const { messages, isThinking, activeTools, sendMessage } = useChat()
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      <Header plan={plan} isAdapting={isAdapting} onAdapt={triggerAdapt} />
      ...
      {infoMessage && (
        <div className="flex items-center gap-3 px-6 py-2 bg-blue-50 border-b border-blue-200 text-sm">
          <Info className="w-4 h-4 text-blue-500 shrink-0" />
          <span className="text-blue-700 flex-1">{infoMessage}</span>
          <button
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
      ...
      <div>
        <h2 className="text-lg font-semibold text-zinc-800">No plan yet</h2>
